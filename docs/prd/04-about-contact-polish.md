...
The About and Contact pages were styled with the same blue-purple-gradient sensibility as the rest of the legacy site and currently fall outside the editorial direction set in `.impeccable.md`. About risks being a bullet-list bio rather than a personal, sequenced narrative; Contact risks being a contact form for a portfolio that does not actually take form submissions. Both pages also load eagerly even though most visitors will spend their time on Portfolio or (after PRD 5) Blog.

## Solution

Treat About as a magazine-style essay page: a single column of long-form copy, a portrait or signature image, optional pull-quotes, and the one reserved use of the Nothing You Could Do script font as a signature accent. Treat Contact as a stripped-down endpoint: a short paragraph, an email address (mailto link with `<app-icon name="envelope">`), the social icons that already live in the Footer (rendered once, not duplicated), and nothing else. Lazy-load both routes.
...
7. As a visitor who wants to reach the photographer, I want a Contact page that gives me a direct email link and nothing more so that I am not asked to fill a form I do not trust.
...
- **Contact layout.** A single short paragraph ("The fastest way to reach me is by email."), a prominent email link rendered as `<app-link external>` style with an envelope icon, and a row of social icons (Instagram, Facebook, LinkedIn) routed through `<app-icon>`. No form. No reCAPTCHA. No name/email/message fields.
- **Email handling.** Use a plain `mailto:` link. If spam protection is later needed, that is a separate concern out of scope here.
- **Topbar wiring.** Ensure the Topbar nav exposes ABOUT and CONTACT links (uncommenting the existing CONTACT line). Add BLOG as a placeholder link in PRD 5; this PRD only restores Contact visibility.
...
- **No new unit tests in this PRD.** Both pages are static composition. Behaviour-rich modules are not introduced.
- **Manual verification checklist:**
  ...
  - Contact renders with a single visible email link, social icons, no form.
  - The mailto link opens the user's mail client.
...
- A contact form with backend handling (no backend exists).
